...
// ==========================================
// NUEVO ENDPOINT: Obtener lista de departamentos para el desplegable
// ==========================================
// Endpoint para alimentar el select dinámico de departamentos
...
// Endpoint estricto para registrar datos en la tabla "usuarios"
app.post('/api/usuarios/registrar', verificarToken, upload.single('foto'), async (req, res) => {
    if (req.user.rol !== 'admin') {
        return res.status(403).json({ error: 'Acceso denegada. Solo administradores.' });
    }

    const { nombre_completo, cedula, correo, celular, departamento_id, rol } = req.body;

    if (!nombre_completo || !cedula || !correo || !departamento_id || !rol) {
        return res.status(400).json({ error: 'Faltan datos obligatorios para el registro.' });
    }

    try {
        // Verificar si la cédula ya existe en la tabla usuarios para evitar duplicados
        const usuarioExistente = await pool.query('SELECT id FROM usuarios WHERE cedula = $1', [cedula]);
        if (usuarioExistente.rowCount > 0) {
            return res.status(400).json({ error: 'La cédula ya está registrada en la tabla usuarios.' });
        }

        // Si subió foto se obtiene la url de Cloudinary, si no, se guarda vacío
        const foto_url = req.file ? req.file.path : '';
        
        // Mapeo exacto de las columnas de tu tabla "usuarios":
        // id (auto), username, cedula, rol, "nombre completo", fecha_ingreso, correo, celular, direccion, foto_url, departamento_id
        const username = cedula; // Se define la cédula como username por defecto
        const fecha_ingreso = new Date();
        const direccion = ''; // Opcional o vacío por defecto

        const query = `
            INSERT INTO usuarios 
            (username, cedula, rol, "nombre completo", fecha_ingreso, correo, celular, direccion, foto_url, departamento_id)
        ...
        res.status(201).json({ message: 'Ok', id: result.rows[0].id });

    } catch (err) {
        console.error('Error al registrar en tabla usuarios:', err);
